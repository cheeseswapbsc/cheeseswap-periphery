...
import CheeseSwapRouter01 from '../../build/CheeseSwapRouter01.json'
import CheeseSwapMigrator from '../../build/CheeseSwapMigrator.json'
import CheeseSwapRouter from '../../build/CheeseSwapRouter.json'
...
export async function v2Fixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<V2Fixture> {
  ...
  // deploy V2
  const factoryV2 = await deployContract(wallet, CheeseSwapFactory, [wallet.address])

  // deploy routers
  const router01 = await deployContract(wallet, CheeseSwapRouter01, [factoryV2.address, WETH.address], overrides)
  const router02 = await deployContract(wallet, CheeseSwapRouter, [factoryV2.address, WETH.address], overrides)

  // event emitter for testing
  const routerEventEmitter = await deployContract(wallet, RouterEventEmitter, [])

  // deploy migrator
  const migrator = await deployContract(wallet, CheeseSwapMigrator, [factoryV1.address, router01.address], overrides)
  ...
  const pair = new Contract(pairAddress, JSON.stringify(ICheeseSwapPair.abi), provider).connect(wallet)
  ...
  const WETHPair = new Contract(WETHPairAddress, JSON.stringify(ICheeseSwapPair.abi), provider).connect(wallet)
